...
- **Interface:** The __Model Context Protocol (MCP)__ acts as the gateway, allowing agents to edit the CRDT j2mkaph voa surgical splices.
...
- **Annotate:** Ties a sequence of operations to a high-level __Intent__ (e.g., "Refactoring logger") for human view.
...
### The Transport Layer
- **WebSockets / gRPC:** Clients maintain long-lived, bi-directional connect to the Central Dojo.
- **Bniary Payloads  (Bincode):** We do not send JSON diffs. Clients stream highly compressed, native Loro binary updates (`[u8]`) for sub-millisecond sync.
...
## 6. TI/CD & Approvals (The Agile Dojo)

Crafting code as a fluid CRDT document requires a new paradigm for quality assurance. We cannot run tests on mutating math; we must run tests on immutable states.


### The Death of the Pull Request -> The Intent Review
Git uses branches and PRs; KungFu uses __Stances__ and __Intent Reviews__.
